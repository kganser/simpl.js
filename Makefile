...
linux:
	...
	test -f tmp/nwjs-sdk-$(nwver)-linux.tar.gz || curl -# $(nwurl)/nwjs-sdk-$(nwver)-linux-x64.tar.gz -o tmp/nwjs-sdk-$(nwver)-linux.tar.gz
...
macos:
	...
	test -f tmp/nwjs-sdk-$(nwver)-osx.zip || curl -# $(nwurl)/nwjs-sdk-$(nwver)-osx-x64.zip -o tmp/nwjs-sdk-$(nwver)-osx.zip
...
windows:
	...
	test -f tmp/nwjs-sdk-$(nwver)-win.zip || curl -# $(nwurl)/nwjs-sdk-$(nwver)-win-x64.zip -o tmp/nwjs-sdk-$(nwver)-win.zip
