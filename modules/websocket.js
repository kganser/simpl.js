simpl.add('websocket', function(modules) {
  ...
  /** websocket: {
        server: function(request:Request, response:Response, callback:ConnectCallback, options=undefined:ServerOptions) -> function(data:ArrayBuffer),
        statusMessage: function(code:number) -> string
      }
      
      WebSocket connections are negotiated using a `request`-`response` pair from an existing HTTP connection. The
      reader function returned by `server` should be returned in the `http` module's `RequestCallback` to initiate a
      websocket connection. */
      
  /** ConnectCallback: function(connection:Connection, protocol:string, extensions:[string, ...]) -> function(message:string|ArrayBuffer)|null
  ...
  /** ServerOptions: {
        ...
        onError=undefined: function(code:number, connection:Connection|undefined)
      }
      ...
      `protocols` and `extensions` are specified in decreasing priority order. If a request is not accepted, or if an
      error occurs on an accepted connection, the connection is closed and `onError` is called with the HTTP or
  ...
  /** Connection: {
      ...
      A connection is automatically closed if terminated by the client, in which case `send` will not issue `callback`.
      `ClientSocket` and `SendInfo` are defined in the `socket` module. */
  return self = {
    server: function(request, response, callback, options) {
      if (!options) options = {};
      var connection, closed, buffer, fin, opcode, mask, length, o,
          ...
          receive = function() {},
          error = function(code) {
            if (connection) connection.close(code);
            else response.generic(code, {
          ...
          if (pos < start) {
            ...
            if (offset+size == length) {
              ...
              if (fin) {
                ...
                else receive(opcode == 1 ? utf8decode(buffer) : buffer);
                buffer = null;
              }
            }
          }
