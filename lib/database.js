kernel.add('database', function() {
  if (typeof WorkerGlobalScope != 'undefined') {
    var proxy = o.proxy();
    return {
      get: function(path, callback) { proxy.send('database_get', [path], callback); },
      put: function(path, value, insert, callback) { proxy.send('database_put', [path, value, insert], callback); },
      append: function(path, value, callback) { proxy.send('database_append', [path, value], callback); },
      delete: function(path, callback) { proxy.send('database_delete', [path], callback); }
    };
  }
